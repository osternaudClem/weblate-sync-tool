...
const getUsedKeys = async (srcPath, translationFunctionNames) => {
  const lexer = new Lexer();

  translationFunctionNames.forEach((translationFunctionName) => {
    const escapedFunctionName = translationFunctionName.replace('.', '\\.');
    lexer.addRule(new RegExp(`^${escapedFunctionName}\\(`), 'FUNCTION_START');
  });
